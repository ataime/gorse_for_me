...
func parseSearchDocumentsResult(result any) (count int64, keys []string, documents []Document, err error) {
	...
	if !ok {
		return
		// return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-1: %#v", result)
	}
	count, ok = rows[0].(int64)
	if !ok {
		return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-2: %#v", rows[0])
	}
	for i := 1; i < len(rows); i += 2 {
		key, ok := rows[i].(string)
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-3: %#v", rows[i])
		}
		keys = append(keys, key)
		row, ok := rows[i+1].([]any)
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-4: %#v", rows[i+1])
		}
		...
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-5: %#v", fields["id"])
		}
		score, ok := fields["score"].(string)
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-6: %#v", fields["score"])
		}
		...
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-7: %#v", fields["categories"])
		}
		...
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-8: %#v", fields["timestamp"])
		}
...
func parseGetTimeSeriesPointsResult(result any) (count int64, keys []string, points []TimeSeriesPoint, err error) {
	rows, ok := result.([]any)
	if !ok {
		return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-9: %#v", result)
	}
	count, ok = rows[0].(int64)
	if !ok {
		return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-10: %#v", rows[0])
	}
	for i := 1; i < len(rows); i += 2 {
		key, ok := rows[i].(string)
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-11: %#v", rows[i])
		}
		keys = append(keys, key)
		row, ok := rows[i+1].([]any)
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-12: %#v", rows[i+1])
		}
		fields := make(map[string]any)
		for j := 0; j < len(row); j += 2 {
			fields[row[j].(string)] = row[j+1]
		...
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-13: %#v", fields["name"])
		}
		value, ok := fields["value"].(string)
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-14: %#v", fields["value"])
		}
		...
		timestamp, ok := fields["timestamp"].(string)
		if !ok {
			return 0, nil, nil, errors.Errorf("invalid FT.SEARCH result-15: %#v", fields["timestamp"])
		}
		timestampMicros, err := strconv.ParseInt(timestamp, 10, 64)
		if err != nil {
			return 0, nil, nil, errors.Trace(err)
